...
const dbPath = path.join(dbDir, 'autolivestream.db');
...
// ---------------------------------------------------------------------------
// FIX: auto-initialize tables on require.
// Previously initializeDatabase() was only EXPORTED, never called, so the
// autolivestream.db tables (including `users`) were never actually created,
// causing "no such table: users" when creating the first admin account.
// ---------------------------------------------------------------------------
initializeDatabase()
  .then(() => {
    // Legacy scheduler table kept for backward compatibility (superset schema).
  ...
  .catch((err) => {
    console.error('Database auto-initialization failed:', err && err.message ? err.message : err);
  });
